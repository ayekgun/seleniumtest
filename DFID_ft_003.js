...
(async function login() {
    ...
    // choose evermos tab
    async function everTab(){
        let tab = await driver.findElement(By.xpath("//div[@id='__layout']/div/div[3]/a/span")).click();
    }

    // click back button
    async function bck(){
        let tab = await driver.findElement(By.xpath("//div[@id='__layout']/div/div[2]/div/div/a/i/span")).click();
    }
    setTimeout(closePopup,10000);
    setTimeout(notifBtn,12000);
    setTimeout(ikhTab,16000);
    setTimeout(everTab,16000);
    setTimeout(ikhTab,17000);
